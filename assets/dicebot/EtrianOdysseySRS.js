...
(function(Opal) {
  ...
  return (function($base, $super, $parent_nesting) {
    ...
    var def = self.$$proto, $nesting = [self].concat($parent_nesting), TMP_EtrianOdysseySRS_initialize_1, TMP_EtrianOdysseySRS_gameName_2, TMP_EtrianOdysseySRS_gameType_3, TMP_EtrianOdysseySRS_getHelpMessage_4, TMP_EtrianOdysseySRS_changeText_6;

    
    self.$setPrefixes(["2D6.*", "EO.*", "SQ.*"]);
    
    Opal.defn(self, '$initialize', TMP_EtrianOdysseySRS_initialize_1 = function $$initialize() {
      var self = this, $iter = TMP_EtrianOdysseySRS_initialize_1.$$p, $yield = $iter || nil, $zuper = nil, $zuper_i = nil, $zuper_ii = nil;

      if ($iter) TMP_EtrianOdysseySRS_initialize_1.$$p = null;
      // Prepare super implicit arguments
      for($zuper_i = 0, $zuper_ii = arguments.length, $zuper = new Array($zuper_ii); $zuper_i < $zuper_ii; $zuper_i++) {
        $zuper[$zuper_i] = arguments[$zuper_i];
      }
      
      $send(self, Opal.find_super_dispatcher(self, 'initialize', TMP_EtrianOdysseySRS_initialize_1, false), $zuper, $iter);
      self.sendMode = 2;
      self.d66Type = 1;
      return (self.sortType = 1);
    }, TMP_EtrianOdysseySRS_initialize_1.$$arity = 0);
    
    Opal.defn(self, '$gameName', TMP_EtrianOdysseySRS_gameName_2 = function $$gameName() {
      var self = this;

      return "世界樹の迷宮SRS"
    }, TMP_EtrianOdysseySRS_gameName_2.$$arity = 0);
    
    Opal.defn(self, '$gameType', TMP_EtrianOdysseySRS_gameType_3 = function $$gameType() {
      var self = this;

      return "EtrianOdysseySRS"
    }, TMP_EtrianOdysseySRS_gameType_3.$$arity = 0);
    
    Opal.defn(self, '$getHelpMessage', TMP_EtrianOdysseySRS_getHelpMessage_4 = function $$getHelpMessage() {
      var self = this;

      return "" + "・判定\n" + "　・通常判定　　　　　　2D6+m>=t[c,f]\n" + "　　修正値m,目標値t,クリティカル値c,ファンブル値fで判定ロールを行います。\n" + "　　クリティカル値、ファンブル値は省略可能です。([]ごと省略できます)\n" + "　　自動成功、自動失敗、成功、失敗を自動表示します。\n" + "\n" + "　　例) 2d6+2>=10       修整+2、目標値10で判定\n" + "　　例) 2d6+2>=10[11]   ↑をクリティカル値11で判定\n" + "　　例) 2d6+2>=10[12,4] ↑をクリティカル値12、ファンブル値4で判定\n" + "　　例) SQ+2>=10        2d6+2>=10と同じ（SQが2D6のショートカットコマンド）\n" + "\n" + "・D66ダイス(入れ替え無し)あり\n"
    }, TMP_EtrianOdysseySRS_getHelpMessage_4.$$arity = 0);
    return (Opal.defn(self, '$changeText', TMP_EtrianOdysseySRS_changeText_6 = function $$changeText(string) {
      var TMP_5, self = this;

      
      string = $send(string, 'gsub', [/^(S)?(EO|SQ)/i], (TMP_5 = function(){var self = TMP_5.$$s || this;

      return "" + (Opal.const_get_relative($nesting, 'Regexp').$last_match(1)) + "2D6"}, TMP_5.$$s = self, TMP_5.$$arity = 0, TMP_5));
      return string;
    }, TMP_EtrianOdysseySRS_changeText_6.$$arity = 1), nil) && 'changeText';
